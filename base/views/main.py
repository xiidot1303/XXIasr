    ...
    if request.user.is_authenticated:
        ...
        if request.method == "POST":
            ...
            if user is not None:
                ...
                if not ip in OFFICE_IP_ADDRESSES and user.username not in USERS_WITHOUT_CONFIRMATION:
                    # send 2FA 
...
def monitoringPage(request):
    ...
    if 'filter' in request.GET:
        ...
        if not date_to:
            date_to = '3000-12-12'
        office = request.GET['office'] if 'office' in request.GET else ''
        status = request.GET['status']
        user = request.GET['user'] if 'user' in request.GET else ''
        service = request.GET['service']
        
        uploads = Upload.objects.filter(Q(period__range=(date_from, date_to)) | Q(period=None))
        if office:
        ...
        uploads = Upload.objects.filter(Q(status=5) | Q(status=0) | Q(status=10))
...
def createClient(request):
    ...
    if 'type' in request.GET:
        ...
        if type == 'student':
            ...
            if request.method == "POST":
                form = StudentCreation(request.POST, request.FILES)
                if form.is_valid():
                        client = form.save(commit=False)
                        client.last_profile = profile
                        client.type = 'student'
        ...
        if type == 'mahalla':
            form = MahallaCreation()
            if request.method == "POST":
                form = MahallaCreation(request.POST, request.FILES)
                if form.is_valid():
                        client = form.save(commit=False)
                        client.last_profile = profile
                        client.type = 'mahalla'
...
def TaxiPage(request):
    ...
    if 'filter' in request.GET:
        ...
        if expiry != "":
            if expiry == "none":
                query = query.filter(Q(ruxsatnoma__exact="") | Q(ruxsatnoma__contains=" ") | Q(ruxsatnoma__isnull=True))
...
@login_required(login_url='login')
def aviakassaPage(request):
    profile = Profile.objects.get(user=request.user)
    clients = Client.objects.filter(type='aviakassa').order_by('id')
    if "page" in request.GET:
        page = request.GET['page']
    else:
        page = 1
    ...
    if 'filter' in request.GET:
        ...
        query = Client.objects.filter(type__exact='aviakassa')
        
        if name != "":
            query = Client.objects.filter(Q(type__exact='aviakassa') & Q(name__icontains=name))
    ...
    return render(request, 'base/aviakassa.html', context)


@login_required(login_url='login')
def daromad12Page(request):
    profile = Profile.objects.get(user=request.user)
    clients = Client.objects.filter(type='daromad12').order_by('id')
    if "page" in request.GET:
        page = request.GET['page']
    else:
        page = 1
    ...
    if 'filter' in request.GET:
        ...
        query = Client.objects.filter(type__exact='daromad12')
        
        if name != "":
            query = Client.objects.filter(Q(type__exact='daromad12') & Q(name__icontains=name))
    ...
    return render(request, 'base/daromad12.html', context)


@login_required(login_url='login')
def taxerPage(request):
    profile = Profile.objects.get(user=request.user)
    clients = Client.objects.filter(type='taxer').order_by('id')
    if "page" in request.GET:
        page = request.GET['page']
    else:
        page = 1
    ...
    if 'filter' in request.GET:
        ...
        query = Client.objects.filter(type__exact='taxer')
        
        if name != "":
            query = Client.objects.filter(Q(type__exact='taxer') & Q(name__icontains=name))
    ...
    return render(request, 'base/taxer.html', context)
...
def studentPage(request):
    profile = Profile.objects.get(user=request.user)
    clients = Client.objects.filter(type='student').order_by('id')
    if "page" in request.GET:
        page = request.GET['page']
    else:
        page = 1
    ...
    if 'filter' in request.GET:
        ...
        query = Client.objects.filter(type__exact='student')
        
        if name != "":
            query = Client.objects.filter(Q(type__exact='student') & Q(name__icontains=name))
...
@login_required(login_url='login')
def mahallaPage(request):
    profile = Profile.objects.get(user=request.user)
    clients = Client.objects.filter(type='mahalla').order_by('id')
    if "page" in request.GET:
        page = request.GET['page']
    else:
        page = 1
    ...
    if 'filter' in request.GET:
        ...
        query = Client.objects.filter(type__exact='mahalla')
        
        if name != "":
            query = Client.objects.filter(Q(type__exact='mahalla') & Q(name__icontains=name))
    ...
    return render(request, 'base/mahalla.html', context)
